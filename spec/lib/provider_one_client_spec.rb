...
RSpec.describe ProviderOneClient do
  let(:to_number) { '1234567777' }
  let(:message) { 'Hello World' }

  context 'when provider one responds with a 200' do
    it 'returns the external message id' do
      VCR.use_cassette("success-provider-one") do
  ...
  context 'when provider is offline' do
    it 'raises an error' do
      VCR.use_cassette("provider-one-is-down") do
        expect { described_class.post(to_number: to_number, message: message) }
          .to raise_error(
            Errors::BadGateway,
